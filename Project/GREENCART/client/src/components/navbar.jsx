...
const Navbar = () => {
  ...
  const { user, setuser, setShowUserLogin, navigate, setSerchQuery, serchQuery ,getCartCount ,axios} = useAppcontext(); // ✅ Fixed: serchQuery lowercase
  ...
  }, [serchQuery]); // ✅ Fixed: correct dependency
        ...
        <div className="absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden flex">
          ...
          {!user ? (
            <button
              ...
              className="px-6 py-2 mt-2 bg-[#4fbf8b] hover:bg-[#44ae7c] transition text-white rounded-full text-sm"
            >
              Login
            </button>
          ) : (
